...
commy releases are **fleet-worker-driveable end-to-end except one step: the PR
merge, which is the human maintainer's**. A worker prepares a release-bump PR;
Graeme merges it; CI does everything outward-facing (tag, npm publish, GitHub
Release) from there. No agent ever pushes a tag or publishes anything.
...
```
WORKER (this runbook)                         GRAEME        CI (release.yml)
─────────────────────                         ──────        ────────────────
...
anything.** The merge is Graeme's, and it triggers everything else.
...
> The live auto-trigger on merge is first exercised on the **next real release**;
> until then it has only been validated by the unit tests on the detection logic
> and a green gate. If the first auto-run misfires, the `workflow_dispatch`
> fallback completes the release.
...
- **The npm publish step stays in `release.yml`.** npm's Trusted Publisher (OIDC)
  ...
  npm reject the publish — that needs a Graeme-side npm-config change. If a
