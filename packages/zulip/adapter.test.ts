...
effectTest('publisher.post hands back message and channel permalinks on the returned ref', () =>
  Effect.gen(function* () {
    ...
    expect(ref.permalink).toBe(
      MessagePermalinkSchema.make('https://zulip.example.com/#narrow/id/99'),
    )
    ...
    expect(Option.map(ref.thread, (t) => t.permalink)).toEqual(
      Option.some(
        ThreadPermalinkSchema.make(
          'https://zulip.example.com/#narrow/channel/1234-general/topic/planning/with/100',
        ),
      ),
    )
  }),
...
effectTest('publisher.post permalink uses the public host header when one is configured', () =>
  Effect.gen(function* () {
    ...
    const ref = yield* adapter.publisher.post(generalChannel.name, decodeMessageBodySync('hi'))
    expect(ref.permalink).toBe(
      MessagePermalinkSchema.make('https://public.zulip.test/#narrow/id/7'),
    )
  }),
)
  ...
  () =>
    Effect.gen(function* () {
      ...
      const parent: MessageRef = {
        ...
        permalink: MessagePermalinkSchema.make('https://zulip.example.com/#narrow/id/1'),
      }
  ...
  Effect.gen(function* () {
    ...
    expect(messages[0]).toEqual({
      ref: {
        ...
        permalink: MessagePermalinkSchema.make('https://zulip.example.com/#narrow/id/555'),
      },
      sender: {
        id: decodeIdentityIdSync('5'),
        name: decodeDisplayNameSync('Robin Reyes'),
...
effectTest('history.messagePermalink builds a link from a channel hint via the streams cache', () =>
  Effect.gen(function* () {
    ...
    expect(link).toEqual(
      Option.some(MessagePermalinkSchema.make('https://zulip.example.com/#narrow/id/77')),
    )
  }),
)
