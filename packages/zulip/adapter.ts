  ...
  Effect.gen(function* () {
    ...
    const mapHistoricalMessage = (
      ...
      Effect.gen(function* () {
        ...
        const { name: cleanName, resolved } = splitTopic(m.subject)
        const threadName = yield* decodeThreadName(cleanName)
        return {
          ref: decorateMessageRef(m.id, channel, {
            name: threadName,
            rawTopic: m.subject,
            resolved,
          }),
    ...
    const decorateMessageRef = (
      id: MessageId,
      channel: ChannelRef,
      thread: ThreadCoordinates | undefined,
    ...
    // The newest message id in a topic, or None when the topic holds no
    ...
    const publisher: MessagePublisher = {
                        ...
                        Effect.map(
                          (id): MessageRef =>
                            decorateMessageRef(
                              id,
                              effective,
                              // The posted topic is the plain thread name, so
                              // rawTopic == name; resolution is unknown here
                              // (post returns only an id) and is left unset.
                              thread === undefined
                                ? undefined
                                : { name: thread.name, rawTopic: thread.name },
                            ),
                        ),
                      ),
                    ),
      ...
      // message in it. `thread` is the plain name; look for the source-state
      // topic first and flip it to the target state, treating an absent source
      // whose target already exists as an idempotent no-op, and an absent
      // both as a thread that isn't there to (un)resolve. Reads route through
      // the minter; the attributed edit goes through the bound identity.
      setThreadResolved: (channel, thread, resolved) =>
        boundHttp().pipe(
          Effect.flatMap((http) => {
            ...
            return findNewestInTopic(channel, sourceTopic).pipe(
              Effect.flatMap(
                Option.match({
                  onSome: (id) =>
                    http
                          ...
                          onNone: () =>
                            Effect.fail(
                              new PublisherError({
                                ...
                                cause: new Error(
                                  `no thread '${thread}' in ${channel.name} to ${
          ...
          Effect.mapError((cause) =>
            cause instanceof PublisherError
              ? cause
              : new PublisherError({ operation: 'setThreadResolved', cause }),
          ),
        ),
    }
                  ...
                  Option.fromNullable(
                    buildMessageRef(
                      ...
                      hint.thread === undefined
                        ? undefined
                        : { name: hint.thread, rawTopic: hint.thread },
                    ).permalink,
                  ),
                ),
              ),
