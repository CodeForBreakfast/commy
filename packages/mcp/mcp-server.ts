 ...
 * field. Covers six topics chosen to keep concierge-style sessions
 ...
 * tool cheat sheet, and the `session_id` contract introduced by
 * ass-2dhb. Kept terse on purpose — connected sessions already carry
 * plenty of CLAUDE.md context.
 */
...
**Tools.** \`post\` (channel; optionally thread or reply), \`react\`/\`unreact\` (emoji on a message), \`subscribe\`/\`unsubscribe\` (live target), \`read_channel\`/\`read_thread\` (history within a range), \`list_channels\` (enumerate channels in the realm), \`resolve\` (identity by name), \`current_identity\` (passive — never acquires), \`download_file\` (fetch a \`/user_uploads/...\` attachment to a temp file — use Read on the returned path to view images), \`upload_file\` (upload a local file by absolute path; returns a \`reference\` string to embed in a \`post\` body).
